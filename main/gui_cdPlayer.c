...
void gui_player_init()
{
    ...
    lv_style_set_text_font(&style1, LV_FONT_DEFAULT);
    ...
    lv_obj_set_style_text_font(lb_albumTitle, LV_FONT_DEFAULT, 0);
    ...
    lv_obj_set_style_text_font(lb_trackTitle, LV_FONT_DEFAULT, 0);
    ...
    lv_obj_set_style_text_font(lb_trackPerformer, LV_FONT_DEFAULT, 0);
    ...
    lv_style_set_text_font(&style_preEmphasis, LV_FONT_DEFAULT);
    ...
    lv_obj_set_style_text_font(lb_time, LV_FONT_DEFAULT, LV_PART_MAIN);
    ...
    lv_obj_set_style_text_font(lb_duration, LV_FONT_DEFAULT, LV_PART_MAIN);
    ...
    lv_obj_set_style_size(chart_left, 0, LV_PART_INDICATOR);
...
void gui_setProgress(uint32_t current, uint32_t total)
{
    ...
    oldCurrent = current; oldToal = total;

    if (total == 0) total = 1234; // avoid divide-by-zero
...
void gui_setTrackNum(int current, int total)
{
    ...
    oldCurrent = current; oldTotal = total;
...
void gui_setMeter(int l, int r)
{
    ...
    if (oldL != l) { oldL = l; lv_bar_set_value(bar_meterLeft,  l, LV_ANIM_OFF); }
    if (oldR != r) { oldR = r; lv_bar_set_value(bar_meterRight, r, LV_ANIM_OFF); }
}
